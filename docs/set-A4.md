...
- [ ] **BUG-07** — make `install-1password.sh`'s guard cover both packages (e.g. `dpkg-query` on `1password` and `1password-cli`), or split the script and correct the success message.
- [ ] **BUG-11** — replace `install-nvm.sh`'s `[ ! -d "$HOME/.nvm" ]` with a check for a usable `$NVM_DIR/nvm.sh`; report the installed version in the closing log.
